...
#create ggplot of the 4 pollutant types showing timeline of all monitors and highlighting the selected monitor
...
#   
#   ggplot(data = selected_dates()) + geom_line(aes(x=Timestamp, y= ., alpha = alpha, group= station_id)) + facet_grid(variable~., scales = "free_y") + guides(alpha = FALSE) + scale_color_manual(values = color)
#   
#   
# })
# plotOutput("pm_25")
...
map_data <- reactive({
  ...
  data[, size := ifelse(station_id == input$sensor, 1.5, 1)]
  data[, stroke := ifelse(station_id == input$sensor,3, 0)]
  data[, color := ifelse(station_id == input$sensor, "red", "gray50")]
  data
